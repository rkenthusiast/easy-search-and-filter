...
use Easy_Search_And_Filter\Engine;

/**
 * Easy_Search_And_Filter Initializer
 */
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'easy_search_and_filter_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'easy_search_and_filter_initialize_failed', $err );
	...
	private function initialize_plugin_class( $classtovalidate ) {
		...
		\add_filter(
			'easy_search_and_filter_instance_' . $classtovalidate,
			function() use ( $temp ) {
				return $temp;
			}
		);
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'Easy_Search_And_Filter\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'Easy_Search_And_Filter\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'Easy Search And Filter is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', ESAF_TEXTDOMAIN ) );
			}
