...
namespace Easy_Search_And_Filter\Rest;
...
class Example extends Base {
	...
	public function get_text_field( array $post_obj ) {
		...
		return \strval( \get_post_meta( $post_id, ESAF_TEXTDOMAIN . '_text', true ) );
	}
	...
	public function update_text_field( string $value, \WP_Post $post, string $key ) {
		...
		if ( false === $post_id ) {
			return new \WP_Error(
				...
				\__( 'Failed to update post views.', ESAF_TEXTDOMAIN ),
				array( 'status' => 500 )
			);
		}
