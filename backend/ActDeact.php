...
class ActDeact extends Base {
	...
	public static function upgrade_procedure() {
		...
		$version = \strval( \get_option( 'easy-search-and-filter-version' ) );

		if ( !\version_compare( ESAF_VERSION, $version, '>' ) ) {
			return;
		}

		\update_option( 'easy-search-and-filter-version', ESAF_VERSION );
		\delete_option( ESAF_TEXTDOMAIN . '_fake-meta' );
	}
