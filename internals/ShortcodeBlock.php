...
class ShortcodeBlock extends \WP_V2_Super_Duper {
	...
	public function __construct() { // phpcs:ignore
		$options = array(
			...
			'block-output'   => array( // the block visual output elements as an array
				array(
					'element' => 'p',
					'title'   => \__( 'Placeholder', ESAF_TEXTDOMAIN ),
					'class'   => '[%className%]',
			...
			'name'           => \__( 'Hello World', ESAF_TEXTDOMAIN ),
			// the name of the widget/block
			'widget_ops'     => array(
				'classname'   => 'hello-world-class',
				// widget class
				'description' => \esc_html__( 'This is an example that will take a text parameter and output it after `Hello:`.', ESAF_TEXTDOMAIN ),
				// widget description
			),
			...
			'arguments'      => array( // these are the arguments that will be used in the widget, shortcode and block settings.
				'after_text' => array( // this is the input name=''
					'title'       => \__( 'Text after hello:', ESAF_TEXTDOMAIN ),
					// input title
					'desc'        => \__( 'This is the text that will appear after `Hello:`.', ESAF_TEXTDOMAIN ),
					// input description
					'type'        => 'text',
	...
	public function initialize() {
		...
		'widgets_init',
		static function() {
			\register_widget( 'Easy_Search_And_Filter\Internals\ShortCodeBlock' );
		}
		);
		*/
	}
